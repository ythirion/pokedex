...
<div class="bg-white rounded-lg shadow-md p-4 mb-6">
	<div class="flex items-center justify-between mb-4">
		...
		<div class="flex gap-2">
			{#if hasActiveFilters}
				<button on:click={clearFilters} class="text-sm text-blue-500 hover:text-blue-700">
					Effacer tout
				</button>
			{/if}
			<button
	...
	<div class:hidden={!isOpen} class="space-y-6 md:block">
		<!-- Generation Filter -->
		<div>
			<label for="generation-select" class="block text-sm font-medium text-gray-700 mb-2">Génération</label>
			<select
		...
		<div>
			<label class="flex items-center gap-2 cursor-pointer">
				<input
					...
					class="w-5 h-5 rounded border-gray-300 text-yellow-500 focus:ring-yellow-500 focus:ring-2"
				/>
		...
		<!-- Type Filters -->
		<div>
			<div class="block text-sm font-medium text-gray-700 mb-2">Types</div>
